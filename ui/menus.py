...
def generate_accounts_flow() -> None:
    ...
    account_name  = _prompt_str("👤 Enter account name prefix")
    ...
    print(f"{get_random_color()}{Colors.BRIGHT}📁 Output:     {settings.ACCOUNTS_FOLDER}{Colors.RESET}")
    ...
    try:
        while any(t.is_alive() for t in threads):
            ...
            print(
                ...
                f"💎 Rare: {settings.RARE_COUNTER} | "
                f"💑 Couples: {settings.COUPLES_COUNTER}"
                f"{Colors.RESET}"
            )
    ...
    if is_ghost:
        print(f"{Fore.LIGHTMAGENTA_EX}{Colors.BRIGHT}📁 GHOST accounts:  {settings.GHOST_ACCOUNTS_FOLDER}{Colors.RESET}")
        print(f"{Fore.LIGHTMAGENTA_EX}{Colors.BRIGHT}💎 Rare GHOST:      {settings.GHOST_RARE_FOLDER}{Colors.RESET}")
        print(f"{Fore.LIGHTMAGENTA_EX}{Colors.BRIGHT}💑 Couples GHOST:   {settings.GHOST_COUPLES_FOLDER}{Colors.RESET}")
    else:
        print(f"{get_random_color()}{Colors.BRIGHT}📁 Accounts:        {settings.ACCOUNTS_FOLDER}{Colors.RESET}")
        print(f"{get_random_color()}{Colors.BRIGHT}💎 Rare accounts:   {settings.RARE_ACCOUNTS_FOLDER}{Colors.RESET}")
        print(f"{get_random_color()}{Colors.BRIGHT}💑 Couples:         {settings.COUPLES_ACCOUNTS_FOLDER}{Colors.RESET}")
        print(f"{get_random_color()}{Colors.BRIGHT}🔐 JWT tokens:      {settings.TOKENS_FOLDER}{Colors.RESET}")
...
def about_section() -> None:
    ...
    for label, path in locations:
        print(f"  {get_random_color()}• {label}: {path}{Colors.RESET}")
...
def main_menu() -> None:
    while True:
        ...
        print(f"  {get_random_color()}3) About{Colors.RESET}")
        ...
        try:
            ...
            if choice == "1":
                ...
                print_error("Invalid option. Choose 1, 2, 3, or 0.")
                time.sleep(1)
        except KeyboardInterrupt:
            safe_exit()
